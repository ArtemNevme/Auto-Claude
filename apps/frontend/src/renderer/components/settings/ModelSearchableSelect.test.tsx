...
describe('ModelSearchableSelect', () => {
  ...
  it('should render with initial value', () => {
    render(
      <ModelSearchableSelect
        value="claude-sonnet-4-6-20250929"
        onChange={mockOnChange}
        baseUrl="https://api.anthropic.com"
        apiKey="sk-test-key-12chars"
      />
    ...
    const input = screen.getByDisplayValue('claude-sonnet-4-6-20250929');
    expect(input).toBeInTheDocument();
  });

  it('should fetch models when dropdown opens', async () => {
    mockDiscoverModels.mockResolvedValue([
      { id: 'claude-sonnet-4-6-20250929', display_name: 'Claude Sonnet 4.6' },
  ...
  it('should display fetched models in dropdown', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('claude-sonnet-4-6-20250929')).toBeInTheDocument();
    });
  });
  ...
  it('should select model and close dropdown', async () => {
    ...
    await waitFor(() => {
      const modelButton = screen.getByText('Claude Sonnet 4.6');
      fireEvent.click(modelButton);
    });

    expect(mockOnChange).toHaveBeenCalledWith('claude-sonnet-4-6-20250929');
  });
  ...
  it('should highlight selected model', async () => {
    ...
    await waitFor(() => {
      ...
      const sonnetButton = screen.getByText('Claude Sonnet 4.6').closest('button');
